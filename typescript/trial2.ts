...
// Callback function 
function sayHi2(callback: ()=>void){ //接收一個Callback function
    callback();
}
...
//計算相同字串的數量並以物件呈現
const arr4: string[] = ['a', 'a', 'b', 'c', 'c', 'c', 'e'];
const reduceArr4: { [key: string]: number } = arr4.reduce((accumulator: { [key: string]: number }, currentValue: string) => {
